...
public class ProduitDemandeeResource {
  ...
  //  @Value("${jhipster.clientApp.name}")
     ...
     */
    @GetMapping("")
    public ResponseEntity<Page<ProduitDemandee>> getAllProduitDemandees(
            @PageableDefault(size = 20) Pageable pageable
    ) {
        log.debug("REST request to get ProduitDemandees by criteria: {}");

        Page<ProduitDemandee> pageProduits = produitDemandeeRepository.findAll(pageable);
